...
import configureStorage from "../lib/multer";
...
export const uploadMultipleFiles = (fieldName: string, maxCount = 5, folderName: string) => {
  const upload = configureStorage(folderName);
  return (req: any, res: Response, next: NextFunction) => {
    upload.array(fieldName, maxCount)(req, res, (err: any) => {
      ...
      req.filePaths = req.files.map(
        (file: any) => `/${file.path.replace(/\\/g, "/")}`
      );
      next();
    });
  };
    ...
    upload.fields(fields)(req, res, (err) => {
      ...
      fields.forEach((field) => {
        ...
        if (req.files[fieldName]) {
          ...
          req.filePaths[fieldName] = req.files[fieldName].map(
            (file: any) => `/${file.path.replace(/\\/g, "/")}`
          );

          // Add file categories for this field
          req.fileCategories[fieldName] = req.files[fieldName].map(
            (file: any) => file.category || "unknown"
          );
